    ...
    for root, dirs, files in os.walk(INPUT_DATA_DIR):
        for filename_with_ext in files:
            ...
            if file_ext in ['.mp4', '.MP4']:
                ...
                output_file = os.path.join(root.replace(INPUT_DATA_DIR, '{}/scale'.format(OUTPUT_DATA_DIR)), '{}{}'.format(filename, file_ext.lower()))
                print('Scaling: {} -> {}'.format(input_file, output_file))
