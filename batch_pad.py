    ...
    for root, dirs, files in os.walk(INPUT_DATA_DIR):
        for filename_with_ext in files:
            ...
            if file_ext in ['.mp4', '.MP4']:
                ...
                output_file = os.path.join(root.replace(INPUT_DATA_DIR, f'{OUTPUT_DATA_DIR}/pad'), f'{filename}{file_ext.lower()}')
                print(f'Padding: {input_file} -> {output_file}')
